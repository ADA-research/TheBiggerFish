...
if __name__ == '__main__':
    ...
    df_LC50 =  pd.read_excel("data/FINAL ECOTOX Internal LC50.xlsx")
    ...
    for index in range(len(folds_setting_3)):
        ...
        x_tr.drop(columns = ['SMILES'], inplace = True); x_val.drop(columns = ['SMILES'], inplace = True); x_te.drop(columns = ['SMILES'], inplace = True)
        ...
        while non_improved < patience:
            ...
            if metrics[1] <= best_val:
                best_val = metrics[1]
                #save the weights
                torch.save(model.state_dict(),'nn params/bestmodelfinetune.pth')
                non_improved = 0
            else:
                non_improved +=1
        ...
        te_info = test_info.iloc[folds_setting_3[index][1]]
        lr = 0.005; finetune_epochs = 2; decay = 0.01
        ...
        for s in x_te['Test organisms (species)'].unique():
            ...
            if s in x_tr['Test organisms (species)'].unique():
                ...
                model.load_state_dict(torch.load('nn params/bestmodelfinetune.pth'))
                for param in model.parameters():
                    param.requires_grad = False
                for param in model.fcout1.parameters():
    ...
    resultsdf.to_csv(f'{global_seed}ECOTOX Finetune Top Internal.csv')